...
const ShowPlayer: React.FC = () => {
  ...
  const fetchVideoThumbnail = (videoId: string) => {
    const youtubeApiKey = "AIzaSyCdvi-PCCAClNngx1OTH-3uoxggXvEhNpA";
    ...
    axios
      ...
      .then((response) => {
        ...
        if (data.items && data.items.length > 0) {
          ...
          if (videoSnippet.thumbnails && videoSnippet.thumbnails.medium) {
            ...
            setVideoInfo({
              title:
                videoSnippet.title.length > 35
                  ? `${videoSnippet.title.slice(0, 35)}...`
                  : videoSnippet.title,
              description:
                videoSnippet.description.length > 60
                  ? `${videoSnippet.description.slice(0, 60)}...`
      ...
      .catch((error) => {
        console.error(
          "Erro ao buscar informações da miniatura do vídeo",
          error
        );
      });
  };
